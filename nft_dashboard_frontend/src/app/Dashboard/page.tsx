...
export default function Dashboard() {
  ...
  const [nftCount, setNftCount] = useState<number | null>(0);
  ...
  useEffect(() => {
    const fetchNFTCount = async () => {
      try {
        ...
        // console.log("NFT Count Response:", data);
    ...
    const fetchCurrentMonthTransactionCount = async () => {
      try {
        const response = await fetch(
          `${CurrentMonthTransactionCount}=${walletAddress}`
        );
        const data = await response.json();
        console.log("Current Month Transaction Count Response:", data);
    ...
    const fetchARCs = async () => {
      try {
        ...
        // Count occurrences of each unique arc_standard type
        ...
        setArcCounts(counts);
        setArcLabels(Object.keys(counts)); // Dynamically set labels
      } catch (error) {
  ...
  // Dummy data
  const stats = [
    { title: "Total NFTs", value: `${nftCount} NFTs` },
    { title: "Total Transactions", value: `${transactionCount} TXNs` },
    {
